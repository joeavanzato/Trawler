function Search-TrustProviderDLL {
...
function Search-SuspiciousCertificates {
	...
	foreach ($cert in $certs){
		...
		if ($loadsnapshot){
			...
			$result = Confirm-IfAllowed $allowtable_certificates $cert.Issuer $cert.Subject $detection
			if ($result){
				continue
			}
		}
